...
Spline = function(options){
	...
	for(var i=0; i<this.length-1; i++){
		...
		this.centers.push({"x":(p1.x+p2.x)/2, "y":(p1.y+p2.y)/2});
	}
...
Spline.prototype.cacheSteps = function(mindist){
	...
	for(var t=0; t<this.duration; t+=10){
		var step = this.pos(t);
		var dist = Math.sqrt((step.x-laststep.x)*(step.x-laststep.x)+(step.y-laststep.y)*(step.y-laststep.y));
		if(dist>mindist){
			steps.push(t);
			laststep = step;
		}
...
Spline.prototype.vector = function(t){
	...
	return {
		...
		speed:Math.sqrt((p2.x-p1.x)*(p2.x-p1.x)+(p2.y-p1.y)*(p2.y-p1.y))
	}
}
